<!-- SIDE PART NA SUMMARY -->
        ...
        <?php  
                  ...
                  $lessvat = ($total / 1.14) * 0.14;
                  $netvat = ($total / 1.14);
                  $addvat = ($total / 1.14) * 0.12;

             endforeach;

//DROPDOWN FOR CUSTOMER
$sql = "SELECT Client_ID, C_name, C_surname
        FROM tblclientinfo
        order by C_name asc";
$res = mysqli_query($db, $sql) or die ("Error SQL: $sql");

$opt = "<select class='form-control'  style='border-radius: 0px;' name='Client' required>
        <option value='' disabled selected hidden>Select Customer</option>";
  while ($row = mysqli_fetch_assoc($res)) {
    $opt .= "<option value='".$row['Client_ID']."'>".$row['C_name'].' '.$row['C_surname']."</option>";
  }
$opt .= "</select>";
// END OF DROP DOWN
        ?>  
